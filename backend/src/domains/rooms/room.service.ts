...
class RoomService {
	...
	public async createIndividualRoom(roomDto: CreateRoomDTO): Promise<number> {
		...
		const isInviteeExist =
			(await this.userRepository.findById(inviteeId)) !== null;
		...
		const inviterRooms =
			await this.roomRepository.getIndividualRoomIdsByUserId(inviterId);
		const inviteeRooms =
			await this.roomRepository.getIndividualRoomIdsByUserId(inviteeId);

		const isShareSameIndividualRoom = inviterRooms?.some(roomId =>
			inviteeRooms?.includes(roomId),
		);
		...
		const entitiesToInsert = createRoomToUserEntities(roomId, [
			inviterId,
			inviteeId,
		]);
