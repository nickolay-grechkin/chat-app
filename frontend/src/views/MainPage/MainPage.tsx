	...
	return (
		<div className="wrapper">
			<div className="sidebar">
				<button
					className="invite_modal_button"
					onClick={() => setIsInviteModalOpened(prevState => !prevState)}
				/>
				...
				{rooms?.map(({ id, lastMessage }) => (
					<div className="room" key={id} onClick={() => handleRoomClick(id)}>
						{lastMessage}
					</div>
				))}
			</div>
				...
				<div className="messages-list">
					{messages &&
						messages.map(({ content, id, userId }) => (
							<Message
								className={`message ${
									userId === 0 ? 'own-message' : 'received-message'
								}`}
								key={id}
								content={content}
							/>
